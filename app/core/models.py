...
#: делает и то и другое, нужны две учётки (ADR-0008).
...
#: Autonomy slider positions. У неё два слоя: администратор задаёт её на весь дом
#: («Агент и инструменты»), человек правит себе («Профиль и агент»), и личное
#: перебивает общее (ADR-0012). Она же — умолчание для инструмента, у которого
...
class User(Base):
    ...
    #: Своя самостоятельность — или NULL, «как в доме». Общую задаёт администратор
    #: в `FamilySettings`, а эта колонка её перебивает: доверие к ассистенту у
    #: домашних разное, и тот, кому переспрашивания мешают, снимает их себе, никого
    #: не трогая (ADR-0012). NULL — это не «ноль», а «своей настройки нет»: человек,
    #: который ничего не выбирал, следует за домом и дальше, в том числе когда
    #: администратор передумает.
    autonomy = Column(Integer, nullable=True)
...
class ToolPolicy(Base):
    """Режим одного инструмента на весь дом; нет строки — «как задаёт самостоятельность».

    Это умолчание семьи: его задаёт администратор один раз для всех, а человек
    правит себе поверх (`UserToolPolicy`, ADR-0012). Одно значение личным не
    перебивается — `MODE_OFF`: инструмент, выключенный здесь, не показывается
    модели ни у кого и включается только тем же экраном.
    """
...
class UserToolPolicy(Base):
    """Режим одного инструмента у одного человека — поверх семейного.

    Таблица отдельная, а не колонка `user_id` в `tool_policies`, потому что это
    разные вещи с разными правами: ту строку заводит администратор на весь дом,
    эту — сам человек (или ассистент с его слов). Одна таблица на двоих означала
    бы «чьё это правило» вычислять по NULL, и первая же выборка «покажи режимы
    семьи» тянула бы за собой чужие личные исключения.
    """
    __tablename__ = "user_tool_policies"
    __table_args__ = (UniqueConstraint("user_id", "tool", name="uq_user_tool_policy"),)
...
class AgentRun(Base):
    ...
    #: осталась от режима «от лица», который ушёл вместе с главой семьи (ADR-0008).
