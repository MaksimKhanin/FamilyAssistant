...
class LLMSettings:
    ...
    #: И отдельно — подбор под ограничения: идеи питания. Ручка осталась, а
    #: умолчание у неё теперь такое же, как у двух других: `off`. Размышление
    #: обещало собранный рацион, а на живых моделях давало минуту ожидания, вдвое
    #: больший счёт и ответ не лучше обычного — и это при том, что чаще всего
    #: мыслей не хватало на сам JSON (ADR-0009). Работу здесь делает промпт и то,
    #: что человек написал о себе, а не размышление.
    #:
    #: Механизм на месте: у кого модель думает быстро и дёшево, ставит `low` или
    #: `medium` и получает прежнее поведение.
    reasoning_plan: str = "off"
...
class AdminSettings:
    ...
    relation: str
    family_name: str
    #: Аварийный сброс пароля администратора на значение из окружения. Держать
...
ENABLED_MODULES: List[str] = [
    name.strip()
    for name in os.environ.get("ENABLED_MODULES", "nutrition,security,memory,relationship,web").split(",")
    if name.strip()
]
