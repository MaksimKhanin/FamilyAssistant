...
KEYWORDS = [
    ...
    # Книга рецептов — раньше и запоминания, и рецепта: «запомни рецепт» это не факт
    # о человеке («запомни») и не просьба расписать («рецепт»), а запись в книгу.
    ("remember_recipe", ("запомни рецепт", "запомни этот рецепт", "запомни блюдо",
                         "запомни это блюдо", "отметь блюдо", "книгу рецептов")),
...
def _arguments_for(name: str, text: str) -> Dict[str, Any]:
    ...
    if name == "remember_recipe":
        cleaned = re.sub(r"^\s*(запомни|отметь)( этот| это)?( рецепт| блюдо)?[,:]?\s*",
                         "", text, flags=re.I).strip(" ?")
        # «Запомни рецепт борща» — рецепта ещё никто не писал, его надо расписать;
        # «запомни блюдо» — человеку нужна отметка, а не текст.
        return {"name": cleaned or text.strip(), "with_recipe": "рецепт" in text.lower()}
