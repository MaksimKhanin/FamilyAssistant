...
#: `auto_from` выше самого смелого положения ползунка (0..3) — «спросит всегда».
#: Нужен там, где одно «да» стирает не одну запись, а месяц их: такое подтверждают
#: и при полной самостоятельности агента. Тумблером на экране «Агент и инструменты»
#: это по-прежнему перебивается — но уже руками и осознанно.
ALWAYS_ASK = 4
