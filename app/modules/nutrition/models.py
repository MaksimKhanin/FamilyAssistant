...
#: Потолок на поправку цифр блюда — тот же приём, что и ACTIVITY_CEILING выше:
#: без него правка вида «999999999999» в поле «Ккал» тихо проходит и уводит
#: дневной и недельный баланс в область, которую никто не ждал увидеть, вместе
#: с комментарием ассистента, посчитанным по той же цифре. Щедрый, но не
#: бесконечный: 20000 ккал и 2000 г белка/жира/углеводов за один приём пищи
#: уже за пределами того, что вообще стоит записывать одной строкой.
MEAL_FIELD_CEILING = {"kcal": 20000, "protein": 2000, "fat": 2000, "carbs": 2000}

#: Доля потолка, начиная с которой комментарий статистики звучит менее уверенно
#: (см. `service.period_stats`): потолок не даёт цифре стать бесконечной, но не
#: отличает «много, но правдоподобно» от «на самом краю» — это должен делать текст.
NEAR_CEILING_RATIO = 0.8
