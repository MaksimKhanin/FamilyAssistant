...
def admin_env(monkeypatch):
    ...
    monkeypatch.setattr(admin, "display_name", "Марина")
    monkeypatch.setattr(admin, "relation", "мама")
    monkeypatch.setattr(admin, "family_name", "Наша семья")
    monkeypatch.setattr(admin, "reset_password", False)
    return admin


def test_empty_database_gets_a_family_and_a_head(db, admin_env):
    created = ensure_admin(db)

    assert created is not None
    assert created.role == ROLE_HEAD
    assert created.display_name == "Марина"
...
def test_running_twice_changes_nothing(db, admin_env):
    ...
    first.display_name = "Марина Ивановна"
    db.commit()

    ensure_admin(db)

    assert db.query(User).count() == 1
    assert db.query(User).one().display_name == "Марина Ивановна"
...
def test_reset_also_restores_the_head_role(db, admin_env, monkeypatch):
    ensure_admin(db)
    admin = db.query(User).one()
    admin.role = "member"
    db.commit()
    ...
    assert db.query(User).one().role == ROLE_HEAD


def test_existing_family_is_left_alone(db, admin_env, head):
