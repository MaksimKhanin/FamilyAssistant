...
def test_the_stats_screen_lists_what_the_chart_is_made_of(as_head, db, member):
    meal(db, member, title="Борщ со сметаной")
...
def test_a_cross_on_the_stats_screen_returns_to_the_stats_screen(as_head, db, member):
    ...
    assert service.get_meal(db, member.id, row.id) is None


def test_the_chat_still_gets_a_reply_and_not_a_redirect(as_head, db, member):
    row = meal(db, member)
...
def test_a_foreign_address_is_not_a_way_back(as_head, db, member):
    row = meal(db, member)
...
def test_the_day_button_clears_exactly_that_day(as_head, db, member):
    from datetime import timedelta

    meal(db, member, title="Сегодняшнее")
    yesterday = meal(db, member, title="Вчерашнее", kcal=200)
    ...
    assert [m.title for m in service.records_for_period(db, member.id, "week")[0].meals] == ["Вчерашнее"]


def test_a_nonsense_day_changes_nothing(as_head, db, member):
    meal(db, member)
    ...
    assert response.status_code == 303
    assert service.period_stats(db, member.id, "day").consumed == 320


def test_the_period_button_clears_both_halves_of_the_balance(as_head, db, member):
    meal(db, member)
    service.log_activity(db, member.id, "walk", 30)
...
def test_nobody_clears_another_persons_days(client, db, member, other):
    """Чужие цифры и так не показываются — стереть их тем более нельзя.

    Второй домашний переключился в шапке на первого: экран ему цифр не покажет
    ...
    client.post("/login", data={"username": other.username, "password": "pw"},
                follow_redirects=False)
    client.cookies.set(ACTING_COOKIE, str(member.id))
    meal(db, member)
