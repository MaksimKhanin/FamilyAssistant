...
def test_the_journal_shows_days_freshest_first_with_their_records(db, member):
    """Столбик графика должен раскрываться в строки, из которых он сложился."""
    today = datetime.utcnow()
    service.create_draft(db, member.id, estimate(kcal=100), eaten_at=today)
    service.create_draft(db, member.id, estimate(kcal=200), eaten_at=today - timedelta(days=2))
    service.log_activity(db, member.id, "walk", 30, happened_at=today)

    days = service.records_for_period(db, member.id, "week")
...
def test_the_journal_keeps_to_its_window(db, member):
    service.create_draft(db, member.id, estimate(), eaten_at=datetime.utcnow() - timedelta(days=9))

    assert service.records_for_period(db, member.id, "week") == []
    assert len(service.records_for_period(db, member.id, "month")) == 1


def test_clearing_one_day_leaves_the_neighbours_alone(db, member):
    today = datetime.utcnow()
    service.create_draft(db, member.id, estimate(kcal=100), eaten_at=today)
    service.create_draft(db, member.id, estimate(kcal=200), eaten_at=today - timedelta(days=1))

    removed = service.clear_day(db, member.id, local_today())

    assert (removed.meals, removed.activity) == (1, 0)
    assert service.period_stats(db, member.id, "week").consumed == 200


def test_clearing_a_period_takes_both_halves_of_the_balance(db, member):
    service.create_draft(db, member.id, estimate(kcal=500))
    service.log_activity(db, member.id, "walk", 30)

    removed = service.clear_period(db, member.id, "week")
...
def test_clearing_only_meals_keeps_the_activity(db, member):
    service.create_draft(db, member.id, estimate(kcal=500))
    service.log_activity(db, member.id, "walk", 30)

    removed = service.clear_period(db, member.id, "week", service.WHAT_MEALS)

    assert (removed.meals, removed.activity) == (1, 0)
    assert service.period_stats(db, member.id, "week").burned == 120


def test_clearing_never_reaches_another_persons_records(db, member, other):
    service.create_draft(db, member.id, estimate(kcal=500))

    assert not service.clear_period(db, other.id, "month")
    assert service.period_stats(db, member.id, "day").consumed == 500


def test_a_removed_meal_takes_its_photo_off_the_disk(db, member, tmp_path):
    """Строка ушла — снимок тарелки уже никто не откроет, а место он занимает."""
    photo = tmp_path / "plate.jpg"
    photo.write_bytes(b"jpeg")
    meal = service.create_draft(db, member.id, estimate(), image_path=str(photo))

    service.delete_meal(db, member.id, meal.id)

    assert not photo.exists()


def test_clearing_a_period_takes_the_photos_too(db, member, tmp_path):
    photo = tmp_path / "plate.jpg"
    photo.write_bytes(b"jpeg")
    service.create_draft(db, member.id, estimate(), image_path=str(photo))

    service.clear_period(db, member.id, "day")

    assert not photo.exists()


def test_nothing_removed_says_so(db, member):
    removed = service.clear_period(db, member.id, "month")
