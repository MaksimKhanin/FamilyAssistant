...
def test_a_task_is_set_by_the_words_of_the_person(db, head, board):
    task = _task(db, head, board)
...
def test_a_task_is_set_on_any_board_you_can_reach(db, head, member, board):
    """Вопрос к общему логу не зависит от владельца доски."""
    knowledge.share_board(db, head.id, board.id, member.id, RIGHT_VIEW)

    task = _task(db, member, board, request="сколько он съел")

    assert task is not None
    assert task.author_id == member.id


def test_a_stranger_does_not_set_a_task_on_a_board_he_cannot_see(db, head, member, board):
    assert _task(db, member, board) is None
    assert stats.list_tasks(db, board.id) == []


def test_a_kind_outside_the_dictionary_is_not_a_task(db, head, board):
    """Считать код умеет по типу из словаря доски — «съеденное вообще» ему не тип."""
    assert _task(db, head, board, kind="еда") is None


def test_the_kind_is_written_the_way_the_dictionary_writes_it(db, head, board):
    assert _task(db, head, board, kind="КОРМЛЕНИЕ").kind == "кормление"


def test_a_sixth_task_does_not_fit_on_one_board(db, head, board):
    """Пять задач — потолок: утренняя сводка не должна стать отчётом."""
    for number in range(stats.MAX_TASKS_PER_BOARD):
        assert _task(db, head, board, request=f"вопрос {number}") is not None

    with pytest.raises(stats.TooManyTasks):
        _task(db, head, board, request="лишний вопрос")

    assert len(stats.list_tasks(db, board.id)) == stats.MAX_TASKS_PER_BOARD


def test_the_ceiling_is_counted_per_board_and_not_per_family(db, head, board):
    other = knowledge.create_board(db, head.id, board.section_id, "Прогулки")
    knowledge.add_event_type(db, head.id, other.id, "прогулка", "мин")
    for number in range(stats.MAX_TASKS_PER_BOARD):
        _task(db, head, board, request=f"вопрос {number}")

    assert _task(db, head, other, request="сколько гуляли", kind="прогулка") is not None


# --- рассылка: только владелец ---------------------------------------------------

def test_only_the_owner_turns_the_broadcast_on(db, head, member, board):
    """Слать уведомления всей семье, ни с кем не согласовав, нельзя."""
    knowledge.share_board(db, head.id, board.id, member.id, RIGHT_EDIT)
    ...
    assert not stats.set_broadcast(db, member.id, task.id, True)
    assert not task.share_all


def test_the_owner_turns_the_broadcast_on_for_a_task_he_did_not_set(db, head, member, board):
    """Задачу поставил допущенный, а рассылку по своей доске включает владелец."""
    knowledge.share_board(db, head.id, board.id, member.id, RIGHT_EDIT)
    task = _task(db, member, board)

    assert stats.set_broadcast(db, head.id, task.id, True)
...
def test_the_model_gets_the_numbers_ready_and_only_formulates(db, head, board):
    """Модель не складывает столбиком: ей приносят посчитанное."""
    _log(db, head, board, "02:50 170", value=170)
    _log(db, head, board, "06:10 180", value=180)
    _log(db, head, board, "09:30 160", value=160)
    task = _task(db, head, board)
    llm = FakeLLM([_said()])
...
def test_an_uncertain_value_does_not_reach_the_regular_figure(db, head, board):
    _log(db, head, board, "02:50 170", value=170)
    _log(db, head, board, "потом ещё немного", value=40, confidence="low")
    task = _task(db, head, board)
...
def test_another_type_of_the_same_board_is_not_added_to_the_figure(db, head, board):
    """Съеденное и потраченное — разные типы: одна задача считает свой."""
    _log(db, head, board, "02:50 170", value=170)
    _log(db, head, board, "срыгнул 30", kind="срыгивание", value=30)
    task = _task(db, head, board)
...
def test_the_series_keeps_one_unit_and_the_phrase_names_the_rest(db, head, board):
    """Ряд — один показатель в одной единице: миллилитры и литры не лягут на одну ось."""
    _log(db, head, board, "02:50 170", value=170)
    _log(db, head, board, "днём 0.2 л", value=0.2, unit="л")
    task = _task(db, head, board)
    llm = FakeLLM([_said()])
...
def test_what_happened_before_the_window_is_not_counted(db, head, board):
    old = _log(db, head, board, "давнее 500", value=500)
...
def test_a_week_task_looks_a_week_back(db, head, board):
    older = _log(db, head, board, "давнее 500", value=500)
    knowledge.entry_events(db, older.id)[0].at = datetime.utcnow() - timedelta(days=3)
    db.commit()
    _log(db, head, board, "02:50 170", value=170)
    task = _task(db, head, board, digest_kind="weekly_review")
...
def test_nothing_counted_is_not_a_figure(db, head, board):
...
def test_the_figure_survives_a_model_that_did_not_answer(db, head, board):
...
def test_a_second_run_on_the_same_day_does_not_double_the_series(db, head, board):
    _log(db, head, board, "02:50 170", value=170)
    task = _task(db, head, board)
...
def test_the_series_dies_with_its_task(db, head, board):
    _log(db, head, board, "02:50 170", value=170)
    task = _task(db, head, board)
    stats.run_task(db, task, llm=FakeLLM([_said()]))
    task_id = task.id

    assert stats.delete_task(db, head.id, task_id)

    assert stats.series(db, task_id) == []


def test_a_task_dies_with_its_board(db, head, board):
    """Показатель не переживает лог, по которому считался."""
    task_id = _task(db, head, board).id
...
def test_a_stranger_does_not_remove_your_task(db, head, member, board):
    knowledge.share_board(db, head.id, board.id, member.id, RIGHT_EDIT)
    task = _task(db, head, board)

    assert not stats.delete_task(db, member.id, task.id)
    assert stats.list_tasks(db, board.id) == [task]
...
def test_the_figure_reaches_the_person_who_asked_for_it(db, head, board):
    _log(db, head, board, "02:50 170", value=170)
    _task(db, head, board)

    assert _digest(db, head) == ["За сутки малыш съел 170 мл."]


def test_without_the_broadcast_the_figure_stays_with_its_author(db, head, member, board):
    ...
    assert _digest(db, head) == []


def test_the_broadcast_reaches_everyone_allowed(db, head, member, board):
    knowledge.share_board(db, head.id, board.id, member.id, RIGHT_VIEW)
    _log(db, head, board, "02:50 170", value=170)
    _task(db, head, board, for_all=True)

    assert _digest(db, member) == ["За сутки малыш съел 170 мл."]


def test_a_revoked_access_takes_the_figure_away_with_the_board(db, head, member, board):
    knowledge.share_board(db, head.id, board.id, member.id, RIGHT_EDIT)
    _log(db, head, board, "02:50 170", value=170)
    _task(db, member, board)
...
def test_everyone_allowed_hears_the_same_number(db, head, member, board):
    ...
    _log(db, head, board, "09:30 180", value=180)      # пока сводка шла к одному
    later = stats.digest_parts(db, member, "morning_digest",
...
def test_a_morning_task_does_not_come_in_the_evening(db, head, board):
    _log(db, head, board, "02:50 170", value=170)
    _task(db, head, board)

    assert _digest(db, head, kind="evening_summary") == []


def test_the_digest_tells_how_many_entries_wait_for_a_clarification(db, head, board):
    """Одна фраза про необъяснённое: неполное не выдаётся за полное (ADR-0002)."""
    _log(db, head, board, "02:50 170", value=170)
    _log(db, head, board, "потом ещё немного", value=40, confidence="low")
    _task(db, head, board)

    parts = _digest(db, head)
...
def test_nothing_to_tell_is_a_silent_digest(db, head, board):
    _task(db, head, board)

    assert stats.digest_parts(db, head, "morning_digest", llm=FakeLLM([])) == []


# --- ряд глазами табло ------------------------------------------------------------

def test_the_series_gives_back_the_days_asked_for(db, head, board):
    task = _task(db, head, board)
    now = datetime.utcnow()
    for days_back in (5, 3, 0):
        entry = _log(db, head, board, f"минус {days_back} дней — 170", value=170)
