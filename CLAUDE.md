...
Пользовательские сценарии проходятся снаружи через `/api/testkit` и
`scripts/testkit.py`: реплики ассистенту, экраны, действия, подтверждения,
расписания — с трейсами, журналом запросов и снимком базы в ответ.
См. `docs/testkit.md`.
